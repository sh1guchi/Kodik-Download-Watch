...
# Все параметры опциональны — при отсутствии берутся значения по умолчанию.
BATCH_DIR = "batch_tmp"        # Каталог для временных файлов и готовых ZIP-архивов
BATCH_JOB_TTL = 3 * 60 * 60    # (В секундах) Через сколько удалять завершённую задачу и её файлы
BATCH_MAX_EPISODES = 500       # Максимум серий в одной задаче пакетной загрузки
BATCH_CLEANUP_INTERVAL = 600   # (В секундах) Как часто фоновый уборщик проверяет просроченные задачи
